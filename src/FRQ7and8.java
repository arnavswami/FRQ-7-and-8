...
ArrayList<String> possibleNames;
 ...
 a.) Write the getHighestYield method, which returns the
 Plot object with the highest yield among the plots in
farmPlots with the crop type specified by the parameter c.
If more than one plot has the highest yield, any of these
plots may be returned. If no plot exists containing the
specified type of crop, the method returns null.
...
class ExperimentalFarm
{
    ...
    b.) Write the sameCrop method, which returns true if
    all the plots in a given column of farmPlots grow the same
